...
\]
\subsection{Form for use in FEM}
For FEM, we would rather avoid the double div term, so consider the VEF sensitivity
\[
\delta J = \bra \phi^\dag , \delta q_0 - \delta \sigma_r \phi \ket + \bra \phi^\dag , \vdiv \left( \delta c \vdiv \left( E \phi \right) \right)  \ket + \int d^2 r \, \left(  \delta \phi 2J^{ \dag out }  - \phi^\dag 2\delta J^{inc} \right) \cdot \vec{n} 
\]
and apply inner product again
\begin{align*}
\delta J = \bra \phi^\dag , \delta q_0 - \delta \sigma_r \phi \ket - \bra \vgrad \phi^\dag , \left( \delta c \vdiv \left( E \phi \right) \right)  \ket &+ \int d^2 r \, \left(  \delta \phi 2J^{ \dag out }  - \phi^\dag 2\delta J^{inc} \right) \cdot \vec{n}  \\
...
\subsection{Idea: $f(,\phi,\phi^\dag,\vec{p},\delta\vec{p}) \approx \delta E$?}
