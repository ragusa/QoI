...
\jcr{in the thesis, remember to explain that the adjoint source is not to be divided by 2 even when the response is siotropic in agle}
...
the forward steady-state SN transport solution on the fly. For time-dependent problems, one possibility to circumvent the storage issues is to employ a VET approach (which only requires
...
\end{equation}
\jcr{sign issue I think in front of Jinc}
\end{subequations}
