...
\usepackage{amsmath, amsthm, amssymb, amsfonts, mathbbol, booktabs, hyperref, graphicx, float, esint, xcolor, subcaption, xspace}
...
While the Sn adjoint sensitivity formulation given by Eq.~\eqref{snSens} provides a first-order accurate method to determine the sensitivity to multiple perturbation scenarios using only one  forward transport solve and one adjoint transport solve, it can quickly run into technical limitations. Specifically for even a one-group time-independent system, all solutions to the forward and adjoint system in space and angle must be stored for retrieval later. 
\jcr{not true, for time-independent problems, as Hayes did, storing the converged
scatttering source is enough to recompute quickly the full angular flux. this is not the case for transient problems}
For a spatially 3-dimensional system, this translates to storing the angular
flux data across 6-dimensions (space/energy/angle).
...
\end{equation}
\end{subequations}
The Eddington Tensor $\Edd$ is then introduced as a simplifying approximation relating the second angular moment term in equation \eqref{1am} to the scalar flux. 
...
\end{equation}
\jcr{not sure what $\BEdd$ is bold. It is really only a scalar.}
...
\jcr{when I see i, I feel that B is a vector but it is not, is it?}

%%%%%%%----------------------------------------------------------------------------------------------
\subsection{Adjoint VET formulation}
%%%%%%%----------------------------------------------------------------------------------------------

The VET formulation necessitates a reformulation of our adjoint.
\jcr{what is ``our'' adjoint?} As shown in Eq.~\eqref{adjForm}, the double divergence term present in the forward equation contributes to a double gradient term in the adjoint equation.
...
For reasons that will become apparent during sensitivity calculations, the boundary condition chosen for the adjoint equation is given in \eqref{adjVETBC}. Unlike for Sn formulation, the VET adjoint equation does not take the form of a VET transport equation.
...
\jcr{ since my BC expressions are different, I get
\begin{equation}
\label{VETQoIAdjUnpDeriv}
\begin{split}
\qoi=&\bra \phi , \scalResp \ket \\
...
+ \sbra \phi^\dag, \isigt \grad \Edd \phi \sket \\
=&\bra \scalSource , \phi^\dag \ket 
- \sbra \phi     , 2J^{\dag,\text{out}} - B \phi^\dag \sket 
+ \sbra \phi^\dag, 2J^{\text{inc}}      - B \phi      \sket
\end{split}
\end{equation}
}
...
\begin{split}
 - \sbra \phi_p, \Edd \cdot \isigt \grad \phi^\dag \sket  + \sbra \phi^\dag, \isigt \div \Edd \phi_p \sket 
=&- \sbra \phi_p, 2J^{\dag,\text{out}} - B \phi^\dag \sket + \sbra \phi^\dag, 2 J_p^{\text{inc}} - B \phi_p \sket \\
=&- \sbra \phi_p, 2J^{\dag,\text{out}} \sket + \sbra \phi^\dag, 2 J_p^{\text{inc}} \sket 
\end{split}
\end{equation}
Finally, we have
\begin{equation}
