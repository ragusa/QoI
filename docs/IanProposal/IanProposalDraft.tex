...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\jcr{I created nice shortcuts for comments from you and me. Let's use them. Every star in the margin denotes a new comment. red is for you. Blue for me.}
\jcr{The explanation below is well written. Make sure it is 100\% your own. Do not  copy-paste from published documents; it's called plagiarism and it is a major offense in the academic setting.} \iwh{ The only thing that I quoted verbatim was the VVUQ process definitions after the next paragraph. I had it cited, but I make the citation a bit more explicit. Is there a better way to cite something when quoting a sort of "definition" of terms like this?}
...
\]
where the superscript ' denotes a perturbed value. Equivalently, one can employ the unperturbed adjoint to obtain
an estimation of the sensitivity coefficient
\[
\delta_p \qoi = \bra u^\dag, \delta_p q - \delta_pA u \ket 
\]
...
Adjoint methods are of particular interest for UQ. In general, adjoint methods provide a mechanism for propagating uncertainty and error in the system variables to the error in the desired quantity of interest. Djoint methods accomplish this in a particularly economical way, sometimes requiring only two differential system solves which can then be used for any combination of sources of error, as opposed to performing an independent solve for each individual error scenario. \iwh{REDO this} 

These adjoint methods have been applied across various complex and time dependent systems. An example of adjoint methods applied to hydrodynamic systems with shocks can be found in Wildey et al. \cite{Wildey}. A more relevant adjoint example to neutron transport occurs in Stripling et al. in the form of reactor burn-up equations \cite{Stripling}.


Application of the adjoint method to time-dependent transport can pose a major technical limitation. In general, the adjoint method would require storing six-dimensional data (the forward angular flux) at each time step. When dealing with high resolution in these six dimensions and many time steps, this can potentially require an unreasonable amount of memory for data storage, rendering the method functionally unusable. As an aside, for the previously mentioned depletion problem, this limitation was resolved by storing the converged flux moments only at defined checkpoints then using interpolation to reconstruct the forward flux for use in the adjoint, however this method does not extend to transport in general \cite{Stripling}.

\jcr{you may say that checkpointing for the scalar flux was employed by Hayes Stripling for depletion problems, but he only needed to store the converged scattering source or equivalently the converged flux moments at optimized checkpoints in order to reconstruct/interpolate data needed
because the depletion problem is a succession of steady states and typically only a few moments are needed whereas one employ many directions}
\iwh{I'm not sure if I have a shorter version of Stripling paper, but (from my reading) it doesn't go into why this checkpointing is acceptable for depletion problems}

A potential solution to the memory requirement for the time-dependent transport adjoint formulation is the use of a quasi-diffusion method to reduce the overall dimensionality of the transport problem, from 6D+time (space, direction, energy, time) to 4D+time (space, energy, time). The method examined in this work is termed  as a ''Variable Eddington Tensor" formulation, and uses the unperturbed forward angular flux to compute the Eddington tensor needed in the quasi-diffusion approach.
...
\end{equation}
The possibly uncertain parameters in this system are: the total and scattering cross sections $\sigt$ and $\sigs$, the volumetric source term $q$, and the external flux incident on the system given by $\psi^{\text{inc}}$. The unknowns (dependent variables) are the angular flux $\psi(\vr,\vO)$ and the scalar flux $\phi(\vr)$ given by
\[
...
\end{equation}
\jcr{in $d\Omega$, there should be no vector sign. it is an infinitesimal area on the unit sphere}
\iwh{fixed}
...
\jcr{in $d\Omega$, there should be no vector sign. it is an infinitesimal area on the unit sphere}
\end{subequations}
\jcr{you have $f$ and $g$ in the RHS above, but not in the bracket notation}
...
\] 
where $\psi_d(\vr) = \psi(\vr, \vO_d)$ and $w_d$ is the angular quadrature weight. This leads to a coupled system of $D$ equations of the form shown in Eq.~\eqref{snFwd}, where the system is coupled through the scattering source term.
\begin{equation}
\label{snFwd}
\vO \cdot \grad \psi + \sigma_{t,p} \psi = \frac{\sigma_{s}}{4 \pi} \phi + q,  \quad \forall \vr \in V
\end{equation}
\begin{equation}
\psi(\vr) = \psi^{\text{inc}}(\vr), \quad \forall \vr \in \partial V^{-}
\end{equation}
...
Now consider perturbations to our system. Specifically perturbations of $\delta \sigt$, $\delta \sigs$, and $\delta q$ to the total cross section, scattering cross section and angular source term respectively. In addition, the incident angular flux is also perturbed by $\delta \psi^{inc}$. These perturbations result is a perturbed solution to the Sn-transport equation $\psi_p$. 
\jcr{first, there is no delta notation in the equation below}
\jcr{second, be very clear. Maybe say that $\psi_p=\psi+\delta \psi$}
\jcr{third, I have added the subscript p to the sigma's as well}
\iwh{Slightly reorganized this section and made the delta-notation a bit more explicit}
\begin{equation}
\label{snFwdPert}
...
\jcr{fix the last steps. I need to see perturbed sigma and psi too and then you say you drop the double delta terms because you only do a first-order expansion}

Using a $\delta$ notation for the perturbed system variables ($\sigma_{s,p} = \sigs + \delta \sigs$ for example), the first order approximation previously mentioned is shown below in Eq.~\eqref{snFwdPertDelta}. A rearrangement of this formulation and be used to substitute in for the $\psi_p$ terms of Eq.~\eqref{snSensPart} to yield the useful perturbed $\qoi$ formulation shown in Eq.~\eqref{snSens}.

\begin{equation}
\label{snFwdPertDelta}
\vO \cdot \grad \psi_p + \sigma_{t} \psi_p + \delta \sigma_{t} \psi = \frac{\sigma_{s}}{4 \pi} \phi_p +\frac{\delta \sigma_{s}}{4 \pi} \phi + q_p + \delta q_p,  \quad \forall \vr \in V
\end{equation}
\begin{equation}
\psi_p(\vr) = \psi^{\text{out}}(\vr) +\delta \psi^{\text{out}}(\vr), \quad \forall \vr \in \partial V^{-}
\end{equation}

\begin{equation}
\label{snSens}
QoI_p = \bra  \scalSource - \delta \sigt \psi + \frac{\delta\sigs}{4 \pi} \phi + \delta \scalSource , \psi^\dag  \ket - \sbra \psi_p, \psi^\dag \sket\\
\end{equation}

Subtraction of the unperturbed $QoI$ expression in Eq.~\eqref{snAdjQoI} supplies a final equation for computing the change in $QoI$ using only the system perturbations and the unperturbed forward and adjoint solutions, removing the need to solve the perturbed forward equation. Furthermore the boundary terms and be split into incoming and outgoing, with the boundary condition of the adjoint eliminating the outgoing portion since $\sbra \delta \psi, \psi^{\dag,\text{out}} \sket_+=0$.

\begin{equation}
\begin{split}
\delta QoI &= \bra - \delta \sigt \psi + \frac{\delta\sigs}{4 \pi} \phi + \delta \scalSource , \psi^\dag  \ket - \sbra \delta \psi, \psi^\dag \sket \\
&= \bra - \delta \sigt \psi + \frac{\delta\sigs}{4 \pi} \phi + \delta \scalSource , \psi^\dag  \ket - \sbra \delta \psi^{\text{inc}}, \psi^\dag \sket_-
\end{split}
\end{equation}
\jcr{not quite correct. check it}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
