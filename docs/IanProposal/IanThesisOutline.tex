...
\subsection{Shielding system, boundary source}
\todo{Simple slab shield between detector and an incident flux source. Begin with a pure absorber shield which has a simple analytical solution, then introduce scattering both directly and through perturbation. This is a convenient test for the incident flux sensitivity.}

\subsection{Shielding system, volumetic source}
\todo{Create a system that is analogous to the boundary source shielding system above, but instead uses a volumetric source on the hot side of the slab, but with the same incident flux on the shielded side. In Sn both of these should yield the same result for source perturbation, since both are ``exact'' for source perturbations. But in VET there is error in source perturbation due to the Eddington/Boundary Eddington perturbation I want to compare this to the boundary source case to see if the error is better/worse/same depending on the form the source takes (volumetric or incident)}

\subsection{No absorption system}
