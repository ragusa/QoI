...
\centering
  \centering
  \includegraphics[width=.98\linewidth]{figures2/blocks.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\centering
  \centering
  \includegraphics[width=.98\linewidth]{figures2/blocks1.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\centering
  \centering
  \includegraphics[width=.98\linewidth]{figures2/blocks2.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\begin{frame}\frametitle{VET Formulation}
 \begin{flushleft}
Where the boundary condition came from \cite{Miften}
\begin{equation}
\begin{split}
2 J^{\text{inc}}(\vr) 
...
\begin{frame}\frametitle{``Blended'' Approach}
 \begin{flushleft}
Form a $\delta \qoi$ inner-product by picking the exact source terms from \textcolor{red}{SN adjoint Eq.~\eqref{snSens}} and cross-section terms from \textcolor{blue}{VET formulation adjoint Eq.~\eqref{VETsens}}
\begin{equation}
\label{Blendsens}
\begin{split}
...
\centering
  \centering
  \includegraphics[width=.98\linewidth]{figures2/blocks3.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\begin{equation}
\begin{split}
QoI + \sbraSN \psi^\dag,  \psi \sketSN  
=& \bra \phi^\dag_p , \angSource \ket \\
=& \bra \phi^\dag_p , - \Edd^\dag : \grad \left( \frac{1}{\sigt}\grad \varphi \right) + \siga \varphi \ket \\
=& \bra 4 \pi \scalResp + 4 \pi\delta \scalResp , \varphi \ket + \bra\div \left( \delta \isigt \div \Edd^\dag \phi^\dag  \right), \varphi \ket \\
&- \bra \delta \siga \phi^\dag , \varphi \ket 
- \sbra \phi_p^\dag, 2J^{\text{inc}} \sket + \sbra \varphi , 2 J^{\dag,\text{out}} - \delta \isigt \div \Edd \phi^\dag \sket  \\
\end{split}
\end{equation} 
        ...
        \begin{flushleft}
             Homogeneous System with Homogeneous Perturbation
          	 \begin{itemize}
			      ...
			      \item $q^\dag=1$ for $4 \leq x \leq 6$
...
\begin{frame}\frametitle{Shielded Incident Flux}
        \begin{flushleft}
             Shielded System
          	 \begin{itemize}
			      \item Shielding $\sigt=1$, $\sigs=0.5$ for $1 \leq x  \leq 2$ (Vacuum else, $\sigt=1E-8$)
			      \item $q=0$, $\psi^{\text{inc}}=1$ at $x=0$
			      \item $q^\dag=1$ for $3 \leq x \leq 4$
			      \item $\sigt$, $\siga$ perturbed in shield
  			 \end{itemize} 
            \end{flushleft}
\begin{figure}[H]
\label{Trial1}
