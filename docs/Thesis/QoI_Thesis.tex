...
%%%%%%%----------------------------------------------------------------------------------------------
%%%%%%%----------------------------------------------------------------------------------------------
\chapter{\uppercase {Adjoint Variable Eddington Tensor Formulation}}
%%%%%%%----------------------------------------------------------------------------------------------
%%%%%%%----------------------------------------------------------------------------------------------
\jcr{title does not seem descriptive enough. is this exclusively about the 'alternate' form? if so, the title should reflect this}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Solution of the quasi-diffusion VET formulations are performed using a Discontinuous Galerkin (DG) method with an interior-penalty approach; see Arnold \cite{Arnold}, for instance. The modifications are required to support quasi-diffusion using $\Edd$ as opposed to simple neutron diffusion. Linear basis functions are used as the finite element basis. As was the case for SN transport, the spatial domain is discretized into 2,000 uniform elements for the results shown in the following sections.
\jcr{``The'' modifications are required. ? This makes people curious. What are they?}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Trial Results}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\jcr{At this point? Will there be more later? Where? The document should be complete. This does not read as complete. How about something simpler: Thus far, we have presented ....}
...
The six \jcr{6? you just said 7} methods were implemented in a MATLAB finite element method (FEM) solver. One-dimensional test cases were run, varying which parameter experienced a perturbation and the magnitude of that perturbation. The results of the four \jcr{4? I thought there were 5 VET-based methods. I am trying to follow but I am confused} different sensitivity methods were analyzed to identify cases where the efficient VET adjoint showed promise as a time and memory efficient method for computing sensitivity. As a representation of this, the \% \qoi response is plotted against the \% change in a given parameter $p$, which are defined as
\begin{subequations}
\begin{equation}
...
\end{equation}
The unperturbed scalar fluxes are shown in Figure~\ref{fig:Flux1}. These plots show both the true scalar flux $\phi$ found by both SN and VET \jcr{obtained using the SN and VET methods} (which should be equal), as well as the ``forward-like'' flux $\varphi$ obtained as the adjoint of the aVET method. Similarly, both methods of $\phi^\dag$ \jcr{both methods of ... what does that mean?} are shown on the same plot as the adjoint-like $\varphi^\dag$ from the VET formulation. Looking at the graphs it becomes clear that $\phi^\dag \neq \varphi^\dag$ and $\phi \neq \varphi$. 
...
For perturbations, the entire system is perturbed uniformly, resulting in another homogeneous system, so therefore the perturbed \qoi can be easily predicted as $\qoi \approx 2 \phi = \frac{q_p}{\sigma_{a,p}}$. \jcr{my version: $\qoi_p \approx (6-4) \phi^\infty_p = 2 \frac{q_p}{\sigma_{a,p}}$}Given the simple form of the expected flux in the center, the derivatives can be taken to get an expected sensitivity.
\begin{subequations}
\begin{equation}
...
The methods of $\delta \qoi$ \jcr{use full English sentences/word.} were tested on this system. Perturbation in the system parameters were taken in the range of $\pm 10 \%$. The \% response results over this range are plotted in Figure~\ref{fig:Trial1}. $\delta \qoi$ values for selected perturbations are given in Table~\ref{TableT1}.
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{T1:sfig4}
\end{subfigure}
\caption{\qoi response to various perturbation scenarios for the homogeneous system under various homogeneous perturbations. For the unperturbed system $q=2$, $\sigt=1$, and $\sigs=1$.\tcr{I doubt this. wrong sigma}}
\label{fig:Trial1}
\end{figure}
...
As a final test, a more varied and complex Reed system is tested. The system is split into 5 regions of unequal length with properties given below. As for perturbations, $\siga$ experiences perturbations in regions 1 and 4, $\sigs$ is perturbed in regions 4 and 5, and $q$ is perturbed in regions 1 and 4. The system has no incident flux. Response is taken in the right most region of the system \tcr{where exactly?}.
\begin{equation*}
\begin{split}
