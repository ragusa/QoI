...
\begin{flushleft}
Adjoint methods provide a mechanism for propagating uncertainty and error in the system variables to the uncertainty in the desired quantity of interest (\qoi). 
\vspace{3mm}
In general adjoint methods 
have the advantages
\begin{itemize}
\item No additional solves required to find the change in \qoi, $\delta$\qoi
\item Change in \qoi is found using a simple inner product
\end{itemize}
\vspace{3mm}
While adjoint methods can greatly reduce the number of system solves required for sensitivity calculations, for transport memory usage may become an issue. To compute $\delta \qoi$, adjoint transport requires storing both the forward angular $\psi$ and adjoint angular $\psi^\dag$ fluxes. This is prohibitive for \tcr{time-dependent transport} where angular fluxes must be stored \tcr{for each timestep}.
\begin{itemize}
...
\begin{flushleft}
\begin{itemize}
\item Present transport equation along with the transport adjoint
\item Formulate a Variable Eddington Tensor (VET) system from the transport equation and corresponding adjoint
\item Refine the VET method in two ways: one leveraging the transport adjoint and another for estimating Eddington perturbation
...
\centering
  \centering
  \includegraphics[scale=0.4]{figures2/RoadMap.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\centering
  \centering
  \includegraphics[scale=0.4]{figures2/RoadMap1.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\begin{frame}\frametitle{\qoi Computed Using Transport}
 \begin{flushleft}
To begin, consider the one-group transport system with isotropic source $q$ and scattering $\sigs$
\begin{subequations}
\begin{equation}
\label{SNfwd}
...
\begin{split}
\qoi_p 
&:=\braSN \psi_p , \angResp \ketSN \\
&=\braSN \psi_p , \tcr{- \vO \cdot \grad \psi^\dag + \sigt \psi^\dag - \frac{\sigs}{4 \pi} \phi^\dag  }\ketSN \\
...
&\tcr{\approx} \braSN  \angSourcepd - \delta\sigt\psi + \frac{\delta \sigs}{4 \pi} \phi
 , \psi^\dag  \ketSN - \sbraSN \psi_p, \psi^\dag \sketSN
\end{split}
\end{equation}
...
&\approx \braSN  \angSourced - \delta\sigt\psi + \frac{\delta \sigs}{4 \pi} \phi
 ...
 , \phi^\dag \ket - \sbraSN \delta \psi, \psi^\dag \sketSN \\
\end{split}
\end{equation}
Pros
\begin{itemize}
...
\item Need to store angular dependent $\psi$ and $\psi^\dag$, which can be prohibitive for time-dependent problems
\end{itemize}
\end{flushleft}
\end{frame}
...
\centering
  \centering
  \includegraphics[scale=0.4]{figures2/RoadMap2.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\begin{equation}
\label{VEFForm}
- \div \left( \frac{1}{\sigt}\div \Edd \phi \right) + \siga \phi = \scalSource \, \vr \in V 
\end{equation}
\begin{equation}
2 J^{\text{inc}} = \BEdd \phi + \vn \cdot \frac{1}{\sigt} \div \Edd \phi \, \vr \in \bound
\end{equation}
\end{subequations}
...
\begin{frame}\frametitle{VET Formulation Adjoint}
 \begin{flushleft}
...
\centering
  \centering
  \includegraphics[scale=0.4]{figures2/RoadMap3.png}
\end{figure}
\end{flushleft}
\end{frame}
...
\begin{split}
QoI_p 
&= \bra \phi^\dag_p , \angSourcepd \ket  - \sbraSN \psi^\dag,  \psi \sketSN  \\
&= \bra \phi^\dag_p , - \Edd^\dag : \grad \left( \frac{1}{\sigt}\grad \varphi \right) + \siga \varphi \ket + \bra \phi^\dag_p , \angSourced \ket - \sbraSN \psi^\dag,  \psi \sketSN  \\
&\tcr{\approx} \bra 4 \pi \scalResp + 4 \pi\delta \scalResp , \varphi \ket + \bra\div \left( \delta \isigt \div \Edd^\dag \tcr{\phi^\dag}  \right), \varphi \ket - \bra \delta \siga \tcr{\phi^\dag} , \varphi \ket  \\
& \quad + \bra \tcr{\phi^\dag} , \angSourced \ket - \sbra \phi_p^\dag, 2J^{\text{inc}} \sket + \sbra \varphi , 2 J^{\dag,\text{out}} - \delta \isigt \div \Edd \phi^\dag \sket  -\sbraSN \psi^\dag,  \psi \sketSN   \\
\end{split}
\end{equation} 
\end{flushleft}
quad \end{frame}
...
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=.98\linewidth]{figures2/23phi.png}
  \label{T1:sfig1}
\end{subfigure}
%
\begin{subfigure}{0.45\textwidth}
  \centering
  \includegraphics[width=.98\linewidth]{figures2/23phia.png}
  \label{T1:sfig2}
\end{subfigure}
\end{figure}
\end{frame}
...
\begin{frame}\frametitle{Shielded Incident Isotropic Flux}
        \begin{flushleft}
             Shielded system with isotropic incident flux
          	 \begin{itemize}
			      \item Shield $\siga=0.5$, $\sigs=0.5$ for $1 \leq x  \leq 2$ (Vacuum else, $\sigt=1E-8$)
			      \item $q=0$, $\psi^{\text{inc}}=1$ at $x=0$
...
\item For source perturbations: VET forward and VET adjoint show minor deviations $\approx 1 \%$.
...
\begin{frame}\frametitle{Shielded Incident Beam}
        \begin{flushleft}
             Shielded system with mono-directional incident beam
          	 \begin{itemize}
			      \item Shield $\siga=0.5$, $\sigs=0.5$ for $1 \leq x  \leq 2$ (Vacuum else, $\sigt=1E-8$)
			      \item $q=0$, $\psi^{\text{inc}}=1$ at $x=0$
			      \item $\qoi = \int_3^4 \phi \, dx$
...
\begin{subfigure}{.5\textheight}
  \centering
  \includegraphics[width=.98\linewidth]{figures2/25sigaSens.png}
  \label{T3:sfig2}
\end{subfigure}
%
\begin{subfigure}{.5\textheight}
  \centering
  \includegraphics[width=.98\linewidth]{figures2/25sigsSens.png}
  \label{T3:sfig3}
\end{subfigure}%
\begin{subfigure}{.5\textheight}
  \centering
...
\begin{frame}\frametitle{Shielded Incident Beam}
 \begin{flushleft}
\begin{itemize}
\item Deviations in the aVET method are made worse with these boundary conditions.
\item Similar behavior as isotropically incident case for other methods.
\item May indicate that the boundary conditions for aVET needs to be reconsidered.
\end{itemize}
\end{flushleft}
\end{frame}
