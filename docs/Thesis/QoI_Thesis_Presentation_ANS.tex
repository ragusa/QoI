...
\begin{flushleft}
CERT\footnote[frame]{NNSA PSAAP-II Program}(Center for Exascale Radiation Transport) - Focus on deterministic transport code which can scale to Exascale regime
\begin{itemize}
...
\item Can we get away with a Quasi-diffusion treatment with the adjoint approach to reduce dimensionality and still obtain the correct $\delta \qoi$ for use in UQ?
\end{itemize}
\end{flushleft}
\end{frame}
...
\begin{frame}\frametitle{\qoi Computed Using Transport}
 \begin{flushleft}
To begin, consider the one-group steady state \tcr{forward} transport system with isotropic source $q$ and scattering $\sigs$
\begin{subequations}
\begin{equation}
\label{SNfwd}
...
\begin{frame}\frametitle{Transport Adjoint and \qoi}
 \begin{flushleft}
To avoid a forward solve for every perturbation case, an \tcr{adjoint} method may be employed.\cite{Marchuk} The adjoint system is defined 
\begin{subequations}
\begin{equation}
...
%\end{frame}
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 \begin{frame}\frametitle{Sensitivity Using Transport Adjoint}
 \begin{flushleft}
...
Pros
\begin{itemize}
\item Do not need transport solve for each perturbation scenario, just \tcr{2 transport solves} to get unperturbed fluxes $\psi$ and $\psi^\dag$
\item Still \tcr{exact for source} (volumetric and boundary) perturbations, $\delta q$ and $\delta \psi^{\text{inc}}$
\end{itemize}
Cons
\begin{itemize}
\item Only first-order perturbation accurate ($\delta \sigs \delta \phi=0$, $\delta \sigt \delta \phi=0$)
\item \tcr{Need to store} angular dependent $\psi$ and $\psi^\dag$, which is prohibitive for time-dependent problems
\end{itemize}
\end{flushleft}
\end{frame}
...
\begin{frame}\frametitle{Sensitivity Using VET Adjoint}
 \begin{flushleft}
Now an assumption is made that the \tcr{$\Edd$ and $\BEdd$ remain unperturbed under system perturbation}. ($\delta \Edd, \delta \BEdd=0$) 
\begin{equation}
\label{VETSensDeriv}
\begin{split}
\qoi_p = &\bra \tcb{\phi_p} , \scalResp \ket \\
       = &\bra \tcb{\phi_p} , - \Edd^u : \left( \grad \isigt \grad \varphi^\dag \right) + \siga \vefadj \ket \\
\tcr{\approx} & \bra \scalSource + \delta \scalSource + \div \delta \isigt \div \left( \Edd^u \tcr{\phi} \right) - \delta \siga \tcr{\phi}, \vefadj \ket - \sbra \tcb{\phi_p}, \Edd^u \cdot \isigt \grad \vefadj \sket \\
&+ \sbra \vefadj, \isigt \div \Edd^u \tcb{\phi_p} \sket \\
=& \bra q, \vefadj \ket  + \bra \delta \scalSource + \div \delta \isigt \div \left( \Edd^u \tcr{\phi} \right)  - \delta \siga \tcr{\phi}, \vefadj \ket \\
& - \sbra \tcb{\phi_p}, \Edd^u \cdot \isigt \grad \vefadj \sket + \sbra \vefadj, \isigt \div \Edd^u \tcb{\phi_p} \sket 
\end{split}
\end{equation}
...
\label{VETsens}
\begin{split}
\delta \qoi =&  \bra \delta \scalSource - \delta \siga \phi, \vefadj \ket  - \bra \delta \isigt \div \left( \Edd^u \phi \right) , \grad \vefadj \ket + \sbra \vefadj, 2 \delta J^{\text{inc}} \sket \\
\end{split}
\end{equation}
Pros
\begin{itemize}
\item Requires only 1 transport solve to get $\Edd^u$ and $\phi$, and 1 scalar VET solve for $\varphi^\dag$
\item No angular fluxes stored, only $\Edd^u$
\end{itemize}
Cons
\begin{itemize}
\item Only first order perturbation accurate (as was transport adjoint)
\item Major assumption that $\delta \Edd=0$, which is not necessarily true. 
\item A consequence is that the method in general is not exact for source perturbations $\delta q$ and $\delta \psi^{\text{inc}}$, which was the case for transport adjoint
\end{itemize}
\end{flushleft}
\end{frame}
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}\frametitle{Refinement Option 1: ``Blended'' Approach}
 \begin{flushleft}
Form a $\delta \qoi$ inner-product by picking the \tcr{exact} source terms from \tcr{transport adjoint Eq.~\eqref{snSens}} and cross-section terms from \tcb{VET formulation adjoint Eq.~\eqref{VETsens}}
\begin{equation}
\label{Blendsens}
\begin{split}
\delta \qoi =&  \tcr{\bra \angSourced , \phi^\dag \ket} \tcb{- \bra \delta \siga \phi, \vefadj \ket - \bra \delta \isigt \div \left( \Edd^u \phi \right) , \grad \vefadj \ket} \tcr{
- \sbraSN \delta \psi^\text{inc}, \psi^\dag \sketSN }\\
\end{split}
\end{equation}
Pros
\begin{itemize}
\item Requires 2 transport solves to get $\Edd^u$, $\phi$ and $\phi^\dag$ then one VET solve for $\varphi^\dag$
...
\centering
 ...
 , \phi^\dag \ket - \sbraSN \delta \psi^{\text{inc}}, \psi^\dag \sketSN$\\ \hline
     VET Adj			&$ \bra \delta \scalSource, \vefadj  \ket - \bra \delta \siga \phi, \vefadj \ket  - \bra \delta \isigt \div \left( \Edd^u \phi \right) , \grad \vefadj \ket + \sbra \vefadj, 2 \delta J^{\text{inc}} \sket$\\ \hline
     Blend		&$\bra \angSourced , \phi^\dag \ket - \bra \delta \siga \phi, \vefadj \ket - \bra \delta \isigt \div \left( \Edd^u \phi \right) , \grad \vefadj \ket 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}\frametitle{Reed Problem: Adjoint flux comparison}
 \begin{flushleft}
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{T3:sfig2}
\end{subfigure}
\caption*{Adjoint fluxes for scatter region \qoi (left) and void region \qoi (right)}
\end{figure}
\end{flushleft}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}\frametitle{Reed Problem: $\delta$QoI comparison}
 \begin{flushleft}
...
\begin{subfigure}{.5\textwidth}
  ...
  \label{T3:sfig4}
\end{subfigure}
\caption*{$\delta \qoi$ values for scatter region \qoi (left) and void region \qoi (right)}
\end{figure}
\end{flushleft}
\end{frame}
...
\vspace{2mm}
\item Attempt to characterize $\delta \Edd$ in more clever ways.
\end{itemize}
\end{flushleft}
\end{frame}
