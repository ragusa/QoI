...
L=0.5; q=10000; T_inf=300; h=16; a=2150; b=200;c=1.05*0; cond = @(T) a./(T+b)+c;
% region of interest
% direc at 0
% 
do_plot = true;
...
if (abs(c)>eps)
    ...
    plot(x,T);
    error('cannot continue with c/=0. no closed form for T(x)');
else
    fh_T = @(x) -b+(b+T_L)*exp(q/2/a*(L^2-x.^2));
    fh_dTdx = @(x) (-q*x)./(cond(fh_T(x)));
    % plot temperature
    if do_plot
        x=linspace(0,L,10000);
        figure(1); 
        subplot(1,2,1); plot(x,fh_T(x)); axis tight; title('Temperature');
%         subplot(1,2,2); plot(x,fh_dTdx(x)); axis tight; title('Temperature');
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% -d/dx (k(T) dphi/dx) = r(x) = 1/L
% dphidx|_0=0 and -k.dTdx|_L=h(phi(L)-phi_inf)=h.phi(L)
%
% int_0^x (eq) = -k.dphi/dx = x/L
% int_x^L (new.eq) => phi(x) = phi(L) + int_x^L  x/k(T(x))/L 
% bc: -k(T(L))  dphidx|_L = 1 = h.phi(L)

phi_L = 1/h;
for i=1:length(x)
    phi(i) = phi_L + integral(@(x) x./(cond(fh_T(x)))/L, x(i),L);
end
% plot adjoint
if do_plot
...
J_adj_vol = q * int_phi;
J_adj_rob = h*T_inf*phi_L;
J_adj = J_adj_vol + J_adj_rob;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% QoI values
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf('Splitting the adjoint values:\n\tvolume  \t%g\n\tRobin bc\t%g\n',...
    J_adj_vol,J_adj_rob);
% check if bc is verified...
-(phi(end)-phi(end-1))/(x(end)-x(end-1))*cond(fh_T(x(end)))-1
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% create perturbation 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% save values
