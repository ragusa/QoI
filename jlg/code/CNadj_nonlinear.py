...
def adjoint(D_t, dD_t, Sigma_t, BC, Q_t, R_t, N, T, N_WIDTH, T_WIDTH, u0, u_for):
	...
	for t in range(T, -1,-1):
		...
		# Create Operators
		on_diagonal, off_diagonal, rhs = make_operators(D, dD, Sigma, BC(t), R, Q, N, T, N_WIDTH, T_WIDTH, u_for[t-1,:])
...
# Q = du/dt - dD/dx du/dx - D d²u/dx²
# u = 1
u0 = np.ones(N+1)
BC = lambda t: (2, 1, 2, 1)
Q = lambda x, t: 0
qoif, errf, qoia, errfa, err, u, us = test_problem('u = 1',u0, D, dD, Sigma, BC, Q, N, T, 1)
# u = t
u0 = np.zeros(N+1)
BC = lambda t: (2, t/T, 2, t/T)
Q = lambda x, t: 1
qoif, errf, qoia, errfa, err, u, us = test_problem('u = t',u0, D, dD, Sigma, BC, Q, N, T, 1/2)
# u = tx
u0 = np.zeros(N+1)
BC = lambda t: (2, 0, 2, t/T)
Q = lambda x, t: -x + (t/T)**3
qoif, errf, qoia, errfa, err, u, us = test_problem('u = t x',u0, D, dD, Sigma, BC, Q, N, T, 1/4)
...
BC = lambda t: (2, 0, 2, 1)
Q = lambda x, t: - 8 * x**4
qoif, errf, qoia, errfa, err, u, us = test_problem('u = x²',u0, D, dD, Sigma, BC, Q, N, T, 1/3)
...
Q = lambda x, t: 2 * (t/T)
qoif, errf, qoia, errfa, err, u, us = test_problem('u = t²',u0, D, dD, Sigma, BC, Q, N, T, 1/3)
# u = t³
N = 1
T = 100
u0 = np.zeros(N+1)
BC = lambda t: (2, (t/T)**3, 2, (t/T)**3)
Q = lambda x, t: 3 * (t/T)**2
qoif, errf, qoia, errfa, err, u, us = test_problem('u = t³',u0, D, dD, Sigma, BC, Q, N, T, 1/4)

# u = x²t²
N = 100
T = 100
u0 = np.zeros(N+1)
BC = lambda t: (2, 0, 2, (t/T)**2)
Q = lambda x, t: 2*(t/T)*x**2 - 8*(t/T)**6 * x**4
qoif, errf, qoia, errfa, err, u, us = test_problem('u = x²t²',u0, D, dD, Sigma, BC, Q, N, T, 1/9)
