...
f = lambda t, u: 1 # derivative function of u
...
for i in range(1,iters):
	u_tilde = u[i-1] + h*f(t(i-1),u[i-1])
	u[i] = u[i-1] + h/2 * (f(t(i-1),u[i-1])+f(t(i), u_tilde))
# Solve for u*
...
fi = np.array([f(t(i),u[i]) for i in range(iters)])
# Solve forward QoI
qoif = sum(h * u * ri)
# Solve adjoint QoI
qoia = sum(h * us * fi) - (fi[-1]*usN - ri[0]*u0)
# Print Results
