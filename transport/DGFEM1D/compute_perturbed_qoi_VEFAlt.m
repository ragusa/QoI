function d_qoi = compute_perturbed_qoi_VEFAlt(use_forward_flux,phia,phi,phiAlt,psia,Ea)
...
[n1,n2]=size(phiAlt);
if n2==1
    phiAlt=reshape(phiAlt,npar.porder+1,npar.nel);
end
...
for iel=1:npar.nel
    ...
    dEdx = (E1-E0)/2;
    delta_qext = dat.sourcePert(my_zone)/ snq.sw;
    Jac   = npar.dx(iel)/2;
    % assemble
    d_qoi = d_qoi + Jac*dot(phia(:,iel), m*ones(2,1)*delta_qext) ;
    d_qoi = d_qoi - Jac*delta_siga*dot(phia(:,iel), m*phiAlt(:,iel));
    ...
    d_qoi = d_qoi - delta_isigtr/Jac*dot(phiAlt(:,iel), k*phia(:,iel));
end
